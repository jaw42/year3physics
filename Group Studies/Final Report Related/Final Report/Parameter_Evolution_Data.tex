...
\label{app:parameter_fit_data}
	\subsection{Faint End Slope ($\alpha$) Data} % (fold)
	\label{sub:alpha_data}
		\begin{table}[h!]
			...
			\end{center}
			\caption{Data highlighting which filters would be useful for observing particular redshift galaxies\cite{Galactic_Astronomy_Binney_Merrifield}\label{tab:filter_characteristics}}
		\end{table}

	% subsection _alpha_data (end)

	\subsection{Normalisation ($\phi^{*}$) Data} % (fold)
	\label{sub:phi_data}
		\begin{table}[h!]
			\begin{center}
			\makebox[\linewidth]{
	...
	\subsection{Characteristic Magnitude ($M^{*}$) Data} % (fold)
	\label{sub:m_data}
		\begin{table}[h!]
			\begin{center}
			\makebox[\linewidth]{
