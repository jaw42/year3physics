...
main(int argc, char* argv[])
{
    ...
    /* Start Grace with a buffer size of 2048, and ask for the whole of the graph to be visible, then open the pipe */
    if (GraceOpenVA("xmgrace",2048,"-nosafe","-noask","-free",NULL) == -1) 
    {
    ...
    for (i = 1; i <= 100 && GraceIsOpen(); i++) {
        ...
        if (i % 10 == 0) {
            GracePrintf("redraw");
        GracePrintf("kill  s0");//Kills data in set s0 so that it can be reassigned.
        /* Need to reassign symbol type and size*/
        GracePrintf("s0 symbol 1");
        GracePrintf("s0 symbol size 0.3");
        GracePrintf("s0 symbol fill pattern 1");
